...
// [ADD] 统一设置媒体资源（数字人视频优先，其次音频），并控制显示/隐藏
function setTTSAndVideo(ttsUrl, videoUrl, playLabel) {
  ...
  if (videoUrl) {
    ...
    showTTSIndicator(playLabel || "正在播放数字人视频...");
    return videoEl;
  } else if (ttsUrl) {
    audioEl.src = ttsUrl;
    showTTSIndicator(playLabel || "正在播放语音...");
    return audioEl;
  } else {
    hideTTSIndicator();
    return null;
