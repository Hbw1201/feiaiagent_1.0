...
def zhipu_conversation(prompt: str, conversation_id: str = None, app_id: str = None, api_key: str = None):
    ...
    # 如果会话不存在错误，尝试重新创建会话
    if "会话不存在" in reply and conversation_id:
        print(f"[DEBUG] 检测到会话不存在错误，尝试重新创建会话")
        new_conversation_id = create_conversation(app_id, api_key)
        if new_conversation_id:
            print(f"[DEBUG] 重新创建会话成功，新ID: {new_conversation_id}")
            # 重新调用
            reply, conv_id = call_zhipu(app_id, api_key, prompt, new_conversation_id)
            return reply, new_conversation_id
        else:
            return "重新创建会话失败", None
