...
def _init_metagpt_if_needed():
    ...
    with _metagpt_lock:
        ...
        try:
            ...
            # 从当前文件位置向上找到feiaiagent_2.0目录
            project_root = current_file.parent.parent  # feiaiagent_2.0/
            metagpt_dir = project_root / "metagpt_questionnaire"
            
            # 如果没找到，尝试其他可能的路径
            if not metagpt_dir.exists():
                # 尝试在feiaiagent_2.0/feiaiagent_2.0/目录下查找
                alternative_root = current_file.parent.parent.parent / "feiaiagent_2.0"
                metagpt_dir = alternative_root / "metagpt_questionnaire"
            ...
            paths_to_add = [str(project_root), str(metagpt_dir.parent)]
            ...
            if not deepseek_key or deepseek_key.startswith("your-"):
                ...
                _metagpt_error = "DEEPSEEK_API_KEY 未配置"
                return False
...
def metagpt_agent_start_conversational():
    ...
    try:
        ...
        # 尝试初始化MetaGPT，如果失败则回退到本地问卷
        metagpt_available = _init_metagpt_if_needed()
        ...
        if metagpt_available:
            try:
                ...
                logger.warning(f"MetaGPT模式失败，回退到本地问卷: {e}")
                # 回退到本地问卷模式
                pass
        
        # 回退到本地问卷模式
        logger.info("使用本地问卷模式作为回退方案")
...
def metagpt_agent_reply_conversational():
    ...
    try:
        ...
        # 检查是否在MetaGPT会话中
        if hasattr(app, 'metagpt_sessions') and session_id in app.metagpt_sessions:
            # MetaGPT模式
            sess = app.metagpt_sessions[session_id]
            ...
            # 本地问卷回退模式
            if not hasattr(app, 'questionnaire_sessions') or session_id not in app.questionnaire_sessions:
                return jsonify({"error": "会话不存在"}), 400
...
def find_next_question_index(current_index: int, answers: dict) -> int:
    """
    Find the index of the next valid question, applying skip logic.
    ...
    while next_index < len(questions_structured):
        ...
        if str(actual_answer) == str(required_value):
            # Dependency met, this is the next question
            return next_index
        else:
            # Dependency not met, skip this question and check the next one
            next_index += 1
