...
# 全局线程池和缓存
_digital_human_thread_pool = None
_digital_human_cache = {}
_cache_lock = threading.Lock()

def get_digital_human_thread_pool():
    """获取数字人线程池"""
    global _digital_human_thread_pool
    if _digital_human_thread_pool is None:
        _digital_human_thread_pool = concurrent.futures.ThreadPoolExecutor(
            max_workers=2,  # 数字人生成资源密集，限制并发数
            thread_name_prefix="digital_human_worker"
        )
    return _digital_human_thread_pool

def cleanup_digital_human_thread_pool():
    """清理数字人线程池"""
    global _digital_human_thread_pool
    if _digital_human_thread_pool:
        _digital_human_thread_pool.shutdown(wait=True)
        _digital_human_thread_pool = None
...
# ---------- 对外主函数 ----------
def generate_digital_human_assets_async(
    text: str,
    prefix: str,
    ...
    """
    异步版本的数字人资产生成函数，使用线程池并行处理TTS和视频生成
    """
    # 检查缓存
    cache_key = f"{text}_{prefix}"
    with _cache_lock:
        if cache_key in _digital_human_cache:
            cached_result = _digital_human_cache[cache_key]
            # 检查缓存的文件是否仍然存在
            if (cached_result[0].exists() and 
                (cached_result[1] is None or cached_result[1].exists())):
                print(f"使用数字人缓存: {cache_key}")
                return cached_result
    
    # 在线程池中执行数字人生成
    thread_pool = get_digital_human_thread_pool()
    future = thread_pool.submit(_generate_digital_human_assets_sync, text, prefix, ffmpeg_path, static_root)
    ...
    try:
        ...
        if result:
            # 缓存结果
            with _cache_lock:
                _digital_human_cache[cache_key] = result
            return result
        return None, None, "", "", ""
    except concurrent.futures.TimeoutError:
        print(f"数字人生成超时: {text[:50]}...")
        return None, None, "", "", ""
    except Exception as e:
        print(f"数字人生成异常: {e}")
        return None, None, "", "", ""

def _generate_digital_human_assets_sync(
    text: str,
    prefix: str,
    ffmpeg_path: str = "",
    static_root: str = "static",
...
def generate_digital_human_assets(
    ...
    默认使用异步版本以提升性能
    """
    return generate_digital_human_assets_async(text, prefix, ffmpeg_path, static_root)
